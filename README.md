...
> [!NOTE]
> If using GitHub, follow the directions for editing files in [your own repository](https://docs.github.com/en/repositories/working-with-files/managing-files/editing-files#editing-files-in-your-repository) to create a new branch and pull request when editing a file. There is no need to fork the repository.
...
3. Ideally, you will have already recompiled the code on your own laptop and **verified that you can do so without any errors before making changes to the server**. If there are errors, adjust the `.md` files as necessary. Note that the paths to linked images or other resources need to be adjusted so that `resource.ext` becomes `resources/resource.ext`.
